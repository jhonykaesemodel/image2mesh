...
hidden_dim = 1050 # ~(input_dim + output_dim) / 2
output_dim = 29 #bicycle is 21 / motorbik 27
learning_rate = 0.001
...
filename_log = os.path.join(path['model'], class2uid[class_model],
                            'model_classifier_idx_b'+str(batch_size)+'_e'+str(num_epochs)+'.log')
...
train_dataset_labels = np.array(dataset_labels[126,:3500]*100, dtype=np.int32).reshape((-1, 1))
test_dataset_labels = np.array(dataset_labels[126,3500:]*100, dtype=np.int32).reshape((-1, 1))
hf.close()
...
integer_encoded = label_encoder.fit_transform(train_dataset_labels.ravel())
integer_encoded = integer_encoded.reshape(len(integer_encoded), 1)
train_dataset_labels = onehot_encoder.fit_transform(integer_encoded)
train_dataset_labels = np.array(train_dataset_labels, dtype=np.float32)
# inverted = label_encoder.inverse_transform([np.argmax(onehot_encoded[0, :])])
print(train_dataset_labels.shape)

integer_encoded = label_encoder.fit_transform(test_dataset_labels.ravel())
integer_encoded = integer_encoded.reshape(len(integer_encoded), 1)
test_dataset_labels = onehot_encoder.fit_transform(integer_encoded)
test_dataset_labels = np.array(test_dataset_labels, dtype=np.float32)
...
filename = os.path.join(path['model'], class2uid[class_model],
                        'model_classifier_idx_b'+str(batch_size)+'_e'+str(num_epochs)+'.pkl')
torch.save(model.state_dict(), filename)
print('Model saved')


''' Get the estimated parameters (indices estimation) '''
...
for i, (train_dataset, train_dataset_labels) in enumerate(train_loader):
    ...
    out_inverted = label_encoder.inverse_transform([np.argmax(outputs.data.cpu().numpy()[0, :])])
    params_train.append(out_inverted)

filename = os.path.join(path['model'], class2uid[class_model],
                        'model_classifier_idx_params_b'+str(batch_size)+'_e'+str(num_epochs)+'_train.mat')
scipy.io.savemat(filename, {'params_train': params_train})
...
for i, (test_dataset, test_dataset_labels) in enumerate(test_loader):
    ...
    # from one hot to number
    out_inverted = label_encoder.inverse_transform([np.argmax(outputs.data.cpu().numpy()[0, :])])
    params_test.append(out_inverted)

filename = os.path.join(path['model'], class2uid[class_model],
                        'model_classifier_idx_params_b'+str(batch_size)+'_e'+str(num_epochs)+'_test.mat')
scipy.io.savemat(filename, {'params_test': params_test})
